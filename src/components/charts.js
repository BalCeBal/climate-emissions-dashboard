...
export function createEmissionChart(annualData, countryToContinent, onNavigate) {
  ...
  svg.append("style").text(`
    ...
    .chart-axis .tick text { fill: #94a3b8; font-size: 15px; }
  ...
  svg.append("text").attr("x", width / 2).attr("y", height - 10).attr("fill", "#ffffff").attr("font-size", "18px").attr("font-weight", "bold").attr("text-anchor", "middle").text("Sum of Emission (MtCO2)");
  svg.append("text").attr("transform", "rotate(-90)").attr("x", -height / 2).attr("y", 20).attr("fill", "#ffffff").attr("font-size", "18px").attr("font-weight", "bold").attr("text-anchor", "middle").text("Temperature Anomaly (°C)");
  ...
  function update(year, level, loc) {
    ...
    const currentScopeName =
      level === "Global"
        ...
        : level === "Continent"
          ? contNames[loc] || loc
          : groupedData.find(d => d[0] === loc)?.[1]?.at(-1)?.Location_Name || loc;

    const emissionTitle =
      level === "Global"
        ? "Global: Emissions vs Temperature Anomaly"
        : level === "Continent"
          ? `${currentScopeName}: Emissions vs Temperature Anomaly`
          : `${currentScopeName}: Emissions vs Temperature Anomaly`;

    svg.select(".year-watermark").text(year);

    svg.selectAll(".emission-title").remove();

    svg.append("text")
      .attr("class", "emission-title")
      .attr("x", margin.left)
    ...
    // Determine maximized state and top 5 by latest emission
    ...
    svg.select("#comet-labels").selectAll("text").data(groupedData, d => d[0])
      .join(
        enter => enter.append("text")
          .attr("x", d => xScale(d[1][d[1].length - 1].Emissions_MtCO2) + 14)
          .attr("y", d => yScale(d[1][d[1].length - 1].Temp_Anomaly) + 4),
        update => update.call(u => u.transition(t)
          .attr("x", d => xScale(d[1][d[1].length - 1].Emissions_MtCO2) + 14)
          .attr("y", d => yScale(d[1][d[1].length - 1].Temp_Anomaly) + 4))
      )
      ...
      .attr("filter", "drop-shadow(0px 2px 2px rgba(0,0,0,0.8))")
      .attr("opacity", d => {
        if (level === "Country" && d[0] !== loc) return 0.0;

        // Apply top-5 restriction only in minimized Continent mode
        if (level === "Continent" && !isMaximized && !top5.has(d[0])) return 0.0;
...
export function renderDualAxisChart(svgElement, annualData, currentYear, level, loc) {
  ...
  const pastData = chartData.filter(d => d.Year <= currentYear);
  ...
  legendGroup.append("text").attr("x", 12).attr("y", 4).attr("fill", "#94a3b8").attr("font-size", "13px").text("Annual Emissions");
  legendGroup.append("circle").attr("cx", 136).attr("cy", 0).attr("r", 6).attr("fill", "#e2737a");
  legendGroup.append("text").attr("x", 148).attr("y", 4).attr("fill", "#94a3b8").attr("font-size", "13px").text("Temperature Anomaly");
}