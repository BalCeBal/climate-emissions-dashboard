...
export function renderSpiral(
  ...
  if (useAbsolute) {
    // Key insight: use per-month-across-all-years averages to get the
    // true seasonal swing. Averaging all values flattens the range at
    // global/continent level because hot and cold months cancel out.
    const monthlyAvgs = d3.range(1, 13).map(m => {
      const monthVals = sortedData
        .filter(d => d.Month === m)
        .map(d => d.value)
        .filter(v => !isNaN(v));
      return d3.mean(monthVals);
    }).filter(v => v !== undefined && !isNaN(v));

    const seasonalMin = d3.min(monthlyAvgs);
    const seasonalMax = d3.max(monthlyAvgs);

    // Add 20% padding so the spiral doesn't touch the innermost ring
    const padding = (seasonalMax - seasonalMin) * 0.2 || 1;
    const domainMin = seasonalMin - padding;
    const domainMax = seasonalMax + padding;
    ...
    tickValues = d3.ticks(domainMin, domainMax, 4);
  ...
  // Title — Hawkins Spiral with location name
  const contNames = {
    GLB: "Global", EUR: "Europe", ASI: "Asia",
    AFR: "Africa", AME: "Americas", OCE: "Oceania"
  };
  ...
  tickValues.forEach(t => {
    ...
    if (r < 0 || r > 300) return; // skip if out of visible range
  ...
  for (const group of yearGroups) {
    ...
    targetGroup.append("path")
      ...
      .attr("fill", "none")
      .attr("stroke", () => {
        const avg = d3.mean(group.plotData, m => m.value);
